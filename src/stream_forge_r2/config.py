...
class KafkaSettings(BaseSettings):
    ...
    input_topic:       str  = Field(
        default="truck.telemetry.raw",       
        alias="KAFKA_INPUT_TOPIC",
        )
    output_topic:      str  = Field(
        default="truck.telemetry.validated",  
        alias="KAFKA_OUTPUT_TOPIC",
        )
    consumer_group:    str  = Field(
        default="stream-forge-r2-week2",     
    ...
    stream_filter_min_temp_c: float = Field(
        default=-50.0,
      alias="STREAM_FILTER_MIN_TEMP_C")
    stream_filter_max_temp_c: float = Field(
        default=120.0,  
        alias="STREAM_FILTER_MAX_TEMP_C")
